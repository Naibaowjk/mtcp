...
mtcp_accept(mctx_t mctx, int sockid, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	TRACE_CONFIG("get mtcp manager\n");
	mtcp = GetMTCPManager(mctx);
	if (!mtcp) {
		return -1;
	}

	TRACE_CONFIG("validate socket\n");
	if (sockid < 0 || sockid >= CONFIG.max_concurrency) {
		TRACE_API("Socket id %d out of range.\n", sockid);
		errno = EBADF;
		return -1;
	...
	TRACE_CONFIG("check socket type\n");
	if (mtcp->smap[sockid].socktype != MTCP_SOCK_LISTENER) {
		errno = EINVAL;
		return -1;
	}
	...
	TRACE_CONFIG("dequeue from accept queue\n");
	...
	TRACE_CONFIG("check whether there is nothing\n");
	if (!accepted) {
		TRACE_CONFIG("[NEVER HAPPEN] Empty accept queue!\n");
		TRACE_ERROR("[NEVER HAPPEN] Empty accept queue!\n");
	}

	TRACE_CONFIG("check whether need to allocate new socket\n");
	if (!accepted->socket) {
		TRACE_CONFIG("try allocate new socket\n");
		...
		const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if ((socket->opts & MTCP_ADDR_BIND) && 
		...
		if (rss_core != mctx->cpu) {
			errno = EINVAL;
			// return -1;
			printf("rss_core(%d) != mctx->cpu(%d)\nsport:%u, dport:%u", rss_core, mctx->cpu, ntohs(dport), ntohs(socket->saddr.sin_port));
			rss_core = mctx->cpu;
		}
	} else {
		if (mtcp->ap) {
